...
import pusty.f0xC.x86.modules.x86ModuleDouble;

public class x64ModuleDouble extends x86ModuleDouble {
	...
	public void nodeCONST_DOUBLE(F0xEventNodeConstParsing event) {
		...
		addDoubleVar(value);
		event.getCurrentList().add("mov reg0, addrname ["+F0xUtil.getDoubleVarName(value)+"]");
	}
	...
	public void nodeVARFIX_LOCALLOADDOUBLE(F0xEventNodeVarFixParsing event) { x64ModuleInteger.nodeVARFIX_LOCALLOAD(event); }
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixLocalStoreDOUBLE, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_LOCALSTOREDOUBLE(F0xEventNodeVarFixParsing event) { x64ModuleInteger.nodeVARFIX_LOCALSTORE(event); }
	
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixGlobalLoadDOUBLE, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_GLOBALLOADDOUBLE(F0xEventNodeVarFixParsing event) { x64ModuleLong.nodeVARFIX_GLOBALLOAD(event); }
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixGlobalStoreDOUBLE, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_GLOBALSTOREDOUBLE(F0xEventNodeVarFixParsing event) { x64ModuleLong.nodeVARFIX_GLOBALSTORE(event); }
