...
public final class QuickLZ
{
	...
	public static byte[] compress(byte[] source, int level)
	{
		...
		if (level == 1) {
			...
			hashtable = new int[HASH_VALUES][];
			for(int i=0;i<HASH_VALUES;i++) hashtable[i] = new int[QLZ_POINTERS_3];
		}
		...
		while (src <= last_matchstart)
		{
			if ((cword_val & 1) == 1)
			{
				if (src > 3 * (source.length >> 2) && dst > src - (src >> 5))
				{
					...
					for(int i=0;i<source.length;i++) d2[DEFAULT_HEADERLEN+i] = source[i];
		...
		d2 = new byte[dst];
		for(int i=0;i<dst;i++) d2[i] = destination[i];
	...
	static public byte[] decompress(byte[] source)
	{
		...
		if ((source[0] & 1) != 1)
		{
			...
			for(int i=headerLen(source);i<size;i++) d2[i] = source[i];
