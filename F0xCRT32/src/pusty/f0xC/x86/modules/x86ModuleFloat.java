...
public class x86ModuleFloat extends F0xModule {
	...
	public void createFloatTable(F0xEventParsing event) {
		if(getInvokes() == 0) return;
		event.getCurrentList().add("");
		event.getCurrentList().add(";Static Float Map");   
	    for(Entry<String,Float> e:getFloatMap().entrySet())
	    	event.getCurrentList().add(e.getKey()+": dd "+e.getValue());
	}
	...
	public void addFloatVar(float f) {
		floatmap.put(F0xUtil.getFloatVarName(f), f);
	}
	...
	public void nodeVARFIX_LOCALLOADFLOAT(F0xEventNodeVarFixParsing event) { x86ModuleInteger.nodeVARFIX_LOCALLOAD(event); }
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixLocalStoreFLOAT, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_LOCALSTOREFLOAT(F0xEventNodeVarFixParsing event) { x86ModuleInteger.nodeVARFIX_LOCALSTORE(event); }
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixGlobalLoadFLOAT, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_GLOBALLOADFLOAT(F0xEventNodeVarFixParsing event) { x86ModuleInteger.nodeVARFIX_GLOBALLOAD(event); }
	
	@F0xEventSubscription(event=F0xEventType.ParseNodeVarFixGlobalStoreFLOAT, priority=F0xEventPriority.NORMAL)
	public void nodeVARFIX_GLOBALSTOREFLOAT(F0xEventNodeVarFixParsing event) { x86ModuleInteger.nodeVARFIX_GLOBALSTORE(event); }
	...
	public void nodeVARFIX_OBJECTLOADFLOAT(F0xEventNodeVarFixParsing event) {
		event.getCurrentList().add("mov reg0d, [reg1+"+x86ModuleInteger.nodeVARFIX_OBJECTOFFSET(event)+"]");
	}
	...
	public void nodeVARFIX_OBJECTSTOREFLOAT(F0xEventNodeVarFixParsing event) {
		event.getCurrentList().add("mov [reg1+"+x86ModuleInteger.nodeVARFIX_OBJECTOFFSET(event)+"], reg0d");
	}
